...
export const BaseLink = React.forwardRef<HTMLAnchorElement, LinkProps>(function BaseLink(props, ref) {
  const {
...
export const Link = (props: ButtonProps) => {
  const { variant, color, href, children, ...other } = props
  return (
    <Button variant={variant || 'text'} color={color || 'primary'} component={BaseLink} noLinkStyle href={href}>
      {children}
    </Button>
  )
}
